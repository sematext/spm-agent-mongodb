...
var mongoConnect = require('./mongodb-connect.js')
...
function MongoDbAgent (cfg) {
  var u = require('url')
  // console.log(cfg)
  var mongoCfg = u.parse(cfg.url)
  this.url = cfg.url
  this.cfg = cfg
  ...
  return new Agent(
    {
      ...
      init: function () {
        ...
        mongoConnect(cfg, function (err, client) {
          if (err) {
            return console.dir(err)
          }
          this.db = client
          this.adminDb = client.db(client.s.options.dbName).admin()
        }.bind(this))
      },
      getMongoStats: function (cbf) {
        ...
        var self = this
        if (!this.adminDb) {
          return cbf(new Error('No DB available: ' + cfg.url))
        }
        this.adminDb.serverStatus(function (err, info) {
          ...
          if (/^2\.\d\.\d/.test(info.version) && !initialized) {
            initialized = true
            logger.warn('MongoDB 2.x is currently not supported, some metrics might be missing')
          }
          if (!err && info && cbf) {
            var summary = {total: 0, failed: 0}
            // transform OpsCounter to a summary
            if (info.metrics.commands) {
        ...
        Object.keys(metricGroups).forEach(function (group) {
          metricGroups[group].forEach(function (prop) {
            if (!isNaN(stats[prop])) {
              ...
              if (warnings < 10) {
                warnings++
                logger.warn('Property not defined ' + prop + ' ' + stats[prop])
              }
            }
          })
        })
