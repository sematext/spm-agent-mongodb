...
var metricsDefinition = {
  ...
  'dur.journaledMB': {calcDiff: false, agg: 'sum'},
  ...
  'dur.writeToDataFilesMB': {calcDiff: false, agg: 'sum'},
  ...
  return new Agent(
    {
      ...
      getDbStats: function (agent) {
        var self = this
        this.db.command({listDatabases: '*'}, function (err, info) {
          if (err) {
            return
          }
      ...
      aggregate: function aggregate (stats) {
        ...
        Object.keys(metricGroups).forEach(function (group) {
          metricGroups[group].forEach(function (prop) {
            // console.log(prop)
            if (!isNaN(stats[prop])) {
              self.agg.update(new Date().getTime(), prop, stats[prop] || 0 * 1.0 || 0, metricsDefinition[prop].calcDiff)
            } else {
