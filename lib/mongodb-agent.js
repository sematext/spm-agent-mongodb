  ...
  return new Agent(
    {
      ...
      start: function (agent) {
        ...
        timerId = setInterval(function () {
          ...
          Object.keys(mGroups).forEach(function (group) {
            var mName = group
            var filters = ['no-filter']
            if (/locks/.test(group)) {
              mName = 'locks'
              filters = [mGroups[group].keys[0].split('.')[1]]
            }
            var metrics = {ts: now, type: 'mongodb', filters: filters, name: mName, fieldInfo: mGroups[group].keys, value: mGroups[group].values, sct: 'APP'}
            console.log(metrics)
      ...
      init: function () {
        ...
        this.db = new Db(config.mongodb.db,
          new Server(config.mongodb.host,
            config.mongodb.port),
        ...
        this.db.open(function (err, db) {
          ...
          self.adminDb.authenticate(config.mongodb.user,
            config.mongodb.passw,
            function (err, result) {
              if (err) {
                console.error(err.toString())
                return
      ...
      getMongoStats: function (cbf) {
        ...
        this.adminDb.serverStatus(function (err, info) {
          ...
          if (!err && info && cbf) {
            ...
            // console.log(locksSummary)
            cbf(flatten(info))
          }
        })
      /*
      this.db.stats(function (e, m) {
        cbf(flatten(m))
      })
      */
      },
