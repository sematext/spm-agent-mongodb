  ...
  return new Agent(
    {
      ...
      init: function () {
        ...
        this.db.open(function (err, db) {
          if (err) {
            logger.error('Error open database: ' + err.toString())
            return
          } else {
            console.log('Connected to ' + mongoCfg.hostname +  ':' + mongoCfg.port )
          }
          self.adminDb.authenticate(
            ...
            function (err, result) {
              if (err) {
                 logger.warn('Authentication failed:' + err.toString() + ' DB: ' + self.adminDb.s.db.databaseName)
                return
              }
            })
        })
